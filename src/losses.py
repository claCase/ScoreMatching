...
def noise_conditional_score_matching(grad: tf.Tensor,
                                     samples: tf.Tensor,
...
def annealed_noise_conditional_score_matching(grad: tf.Tensor,
                                              samples: tf.Tensor,
                                              corrupted_samples: tf.Tensor,
    ...
    loss = tf.reduce_sum(0.5 * (grad - targets) ** 2, -1) * sigmas[None, :] ** 2  # B x |σ|
    loss = tf.reduce_mean(loss)
    return loss
...
def noise_conditional_score_matching_loss(grad, samples, corrupted_samples, sigmas):
    if isinstance(sigmas, float):
        tf.debugging.assert_shapes([(grad, ('B', 'd')),
                                    (samples, ('B', 'd')),
                                    (corrupted_samples, ('B', 'd'))])
        return noise_conditional_score_matching(grad, samples, corrupted_samples, sigmas)
    elif isinstance(sigmas, tuple) or isinstance(sigmas, tf.Tensor):
        tf.debugging.assert_shapes([(grad, ('BS', 'd')),
                                    (corrupted_samples, ('BS', 'd')),
                                    (samples, ('B', 'd'))])
        return annealed_noise_conditional_score_matching(grad, samples, corrupted_samples, sigmas)
