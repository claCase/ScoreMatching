...
# Use Direct Gradient estimation
...
# Use Energy Based Model
ebm = models.EBMSlicedScoreMatching(hidden_layers=(512, 512))
ebm.compile("adam")
ebm.fit(x, epochs=1000, batch_size=500)

fig, ax = plt.subplots(2, figsize=(10, 20))
fig, ax0 = utils.make_distribution_grad_plot(mix, fig=fig, ax=ax[0])
ax0.scatter(x[:, 0], x[:, 1], color="black", s=5)
ax1 = utils.make_grad_plot(ebm, ax=ax[1], fig=fig)
fig.tight_layout()
